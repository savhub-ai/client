...
Savhub 是一个 AI 编程技能的包管理器。它扫描公开 Git 仓库中的 `SKILL.md` 文件，将它们组织为可安装的技能集（flock），通过一条命令即可安装到任何项目中。支持 Claude Code、Codex、Cursor、Windsurf 等 AI 编辑器。

平台分为两部分：

- **savhub.ai** — 在线注册中心，用于浏览、搜索、收藏和评论技能。
- **Savhub Client** — CLI（及桌面应用），自动检测项目类型并将对应技能同步到你的 AI 编辑器。
