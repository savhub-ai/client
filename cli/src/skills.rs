...
pub fn read_skill_origin(skill_folder: &Path) -> Result<Option<SkillOrigin>> {
    for path in origin_paths(skill_folder) {
        let Ok(raw) = fs::read_to_string(&path) else {
            continue;
        };
        let Ok(origin) = serde_json::from_str::<SkillOrigin>(&raw) else {
            continue;
        };
        if origin.version == 1 {
...
pub fn write_skill_origin(skill_folder: &Path, origin: &SkillOrigin) -> Result<()> {
    let path = skill_folder.join(".savhub").join("origin.json");
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)?;
    }
...
fn origin_paths(skill_folder: &Path) -> [PathBuf; 1] {
    [skill_folder.join(".savhub").join("origin.json")]
}
